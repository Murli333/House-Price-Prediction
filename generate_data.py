...
df["Price"] = (
    df["Area_sqft"] * base_price_per_sqft                   # Core driver: area
    + df["Bedrooms"] * 50_000                                # Small bedroom premium
    + df["Bathrooms"] * 30_000                               # Small bathroom premium
    - df["Age"] * 20_000                                     # Depreciation per year
    + df["Location_Score"] * df["Area_sqft"] * 200           # Location scales with area
    + df["Garage"] * 100_000                                 # Flat garage premium
    + np.random.normal(0, 200_000, n)                        # Noise
).clip(lower=500_000)                                        # Floor: ₹5 lakh minimum
